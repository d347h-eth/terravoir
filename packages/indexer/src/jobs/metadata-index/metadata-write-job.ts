...
export default class MetadataIndexWriteJob extends AbstractRabbitMqJobHandler {
  ...
  public async process(payload: MetadataIndexWriteJobPayload) {
    // Focus-mode gate: only write metadata for the focus contract/collection
      ...
      `
        ...
        UPDATE tokens SET
          ...
          metadata_version = CASE
            WHEN (SELECT is_updated FROM updated_check) THEN COALESCE(metadata_version, 0) + 1
        ...
        WHERE tokens.contract = $/contract/
          AND tokens.token_id = $/tokenId/
        RETURNING collection_id, created_at, image, name, floor_sell_id, (SELECT is_updated FROM updated_check) AS is_updated
      `,
      {
        contract: toBuffer(contract),
        tokenId,
        name: _.isNull(name) ? null : sanitizeText(`${name}`),
        description: sanitizeText(description) || null,
        image: sanitizeText(imageUrl) || null,
        tokenURI: sanitizeText(tokenURI) || null,
        decimals: decimals || null,
        metadata:
          {
            original_metadata: originalMetadata || null,
            image_original_url: sanitizeText(imageOriginalUrl) || null,
            image_mime_type: imageMimeType || null,
            image_properties: imageProperties || null,
            animation_original_url: sanitizeText(animationOriginalUrl) || null,
            media_mime_type: mediaMimeType || null,
            metadata_original_url: sanitizeText(metadataOriginalUrl) || null,
          } || {},
        media: sanitizeText(mediaUrl) || null,
      }
    );
    ...
    if (
      ...
      logger.info(
        this.queueName,
        JSON.stringify({
          message: `New collection for token. contract=${contract}, tokenId=${tokenId}, currentCollectionId=${result.collection_id}, newCollectionId=${collection}`,
          contract,
          token: `${contract}:${tokenId}`,
          newCollectionId: collection,
          currentCollectionId: result.collection_id,
          tokenCreatedAt: new Date(result.created_at).toISOString(),
        })
      );
    ...
    if (!_.isEmpty(attributesToRefresh)) {
      if (!result.is_updated) {
        logger.log(
          ...
          JSON.stringify({
            topic: "tokenMetadataIndexing",
            message: `Only attributes changed. collection=${collection}, tokenId=${tokenId}, metadataMethod=${metadataMethod}`,
            payload,
            metadataHash,
            metadataMethod,
            debugMetadataIndexingCollection:
        ...
        // Update the metadata version in case there was no change in the token level metadata but the attributes changed.
        await idb.oneOrNone(
          `
                UPDATE tokens
                SET metadata_version = COALESCE(metadata_version, 0) + 1,
                    updated_at = now()
                WHERE contract = $/contract/
                AND token_id = $/tokenId/
            `,
