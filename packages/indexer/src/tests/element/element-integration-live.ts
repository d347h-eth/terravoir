...
import type { OrderInfo as ElementOrderInfo } from "@/orderbook/orders/element";
...
const defaultOperator = Wallet.createRandom();
const defaultOperator2 = Wallet.createRandom();

const testNFTAddr =
  (process.env.ELEMENT_TEST_ERC721_ADDRESS ?? "0x0000000000000000000000000000000000000000").toLowerCase();
const erc1155NFT =
  (process.env.ELEMENT_TEST_ERC1155_ADDRESS ?? "0x0000000000000000000000000000000000000000").toLowerCase();
const operatorKey = process.env.ELEMENT_TEST_OPERATOR_KEY ?? defaultOperator.privateKey;
const operator2Key = process.env.ELEMENT_TEST_OPERATOR2_KEY ?? defaultOperator2.privateKey;

export const registerElementIntegrationSuite = () => {
  const operatorProvider = new Wallet(operatorKey, baseProvider);
  const operator2Provider = new Wallet(operator2Key, baseProvider);
  const elementOrders =
    require("@/orderbook/orders/element") as typeof import("@/orderbook/orders/element");
  ...
  test("sellERC721", async () => {
    ...
    const sellOrder = builder.build({
      ...
      tokenId: tokenId,
      paymentToken: Element.Addresses.NativeEthAddress,
      price,
      hashNonce,
    ...
    const orderInfo: ElementOrderInfo = {
    ...
    await elementOrders.save([orderInfo]);
  ...
  test("sellERC1155", async () => {
    ...
    // Store order to database
    const result = await elementOrders.save([orderInfo]);
