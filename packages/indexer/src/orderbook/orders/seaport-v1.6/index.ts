...
export const save = async (
  ...
  const handleOrder = async (
    ...
    try {
      ...
      // Check: order has a supported conduit (fallback: refresh channels once and retry)
      if (
        !(await isOpen(order.params.conduitKey, Sdk.SeaportV16.Addresses.Exchange[config.chainId]))
      ) {
        try {
          const conduit = new Sdk.SeaportV16.Exchange(config.chainId).deriveConduit(
      ...
      // Ensure royalty-enforced orders can only come from OpenSea
      ...
      const erc721cConfigV3 = await erc721c.v3.getConfigFromDb(info.contract);
      if (erc721cConfigV3) {
        if (
          erc721cConfigV3.authorizers.includes(
            Sdk.SeaportBase.Addresses.OpenSeaV16SignedZone[config.chainId]
          ) &&
          order.params.zone === Sdk.SeaportBase.Addresses.OpenSeaV16SignedZone[config.chainId] &&
          !isOpenSea
        ) {
          return results.push({
            id,
      ...
      if (order.params.orderType > 1) {
        if (
          ![
            ...
            // Reservoir royalty-enforcing zone
      ...
      try {
        ...
        if (error.message === "no-balance-no-approval") {
          ...
          logger.debug(
            ...
            `Not fillable. orderId=${id}, error=${error.message}`
          );
      ...
      if (openSeaOrderParams && openSeaOrderParams.kind !== "single-token") {
        let collectionId = openSeaOrderParams.contract;

        if (isSharedContract(openSeaOrderParams.contract)) {
      ...
      const feeBreakdown = info.fees.map(({ recipient, amount }) => {
        ...
        // For non-opensea orders force royalty if the recipient is not in the known collection royalties
        if (
          !isOpenSea &&
          kind === "marketplace" &&
          _.includes(
            defaultRoyalties.map((defaultRoyalty) => defaultRoyalty.recipient),
            recipient.toLowerCase()
          )
        ) {
          logger.info(
            "orders-seaport-v1.6-save",
            JSON.stringify({
              message: `Collection royalty enforcement. orderId=${id}, ${recipient.toLowerCase()} forced to be a royalty`,
              defaultRoyalties,
            })
          );
          ...
          try {
            FeeRecipients.fixFeeRecipients([recipient.toLowerCase()]);
          } catch (error) {
            logger.error(
              "orders-seaport-v1.6-save",
              JSON.stringify({
                message: `Collection royalty enforcement error. orderId=${id}, ${recipient.toLowerCase()}`,
                defaultRoyalties,
                error,
              })
            );
      ...
      // Check: royalty enforcement
      const royaltyEnforcingZone =
        Sdk.SeaportBase.Addresses.ReservoirV16RoyaltyEnforcingZone[config.chainId];
      if (
        royaltyEnforcingZone &&
        erc721cConfigV3?.authorizers.includes(royaltyEnforcingZone) &&
        !isOpenSea
      ) {
        let royaltiesEnforced = true;

        for (const r of defaultRoyalties) {
          if (
            !feeBreakdown.find(
              // Allow a difference of 1bps to cover any inaccuracies when computing the bps from the raw amount
              (f) => f.recipient.toLowerCase() === r.recipient.toLowerCase() && f.bps >= r.bps - 1
            )
          ) {
        ...
        //  Check per-token royalties
        if (!royaltiesEnforced && info.side === "sell") {
        ...
        if (!royaltiesEnforced) {
          return results.push({
            id,
            status: "royalties-not-enforced",
          });
        }
      }
      ...
      if (info.side === "buy" && order.params.kind === "single-token" && validateBidValue) {
        ...
        try {
          let hasCollectionTopBidOrFloorAsk = false;
          ...
          if (collectionTopBidValue) {
            hasCollectionTopBidOrFloorAsk = true;

            if (Number(value.toString()) <= collectionTopBidValue) {
              return results.push({
                id,
                status: "bid-too-low-collection-top-bid",
              });
            }
          } else {
            ...
            if (collectionFloorAskValue) {
              ...
              if (percentage < seaportBidPercentageThreshold) {
                return results.push({
                  id,
                  status: "bid-too-low-collection-floor-ask",
                });
              }
            }
          }

          if (!hasCollectionTopBidOrFloorAsk) {
            // logger.debug(
            //   "orders-seaport-v1.6-save",
            //   `Bid value validation - noCollectionTopBidOrFloorAsk. orderId=${id}, contract=${info.contract}, tokenId=${tokenId}`
            // );
          }
        } catch (error) {
          logger.warn(
      ...
      if (
        ...
        if (
          ...
          [
            Sdk.SeaportBase.Addresses.ReservoirV16CancellationZone[config.chainId],
            Sdk.SeaportBase.Addresses.ReservoirV16RoyaltyEnforcingZone[config.chainId],
          ].includes(replacedOrderResult.raw_data.zone)
        ) {
