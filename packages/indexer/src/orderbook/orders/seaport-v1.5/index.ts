...
export const save = async (
  ...
  const handleOrder = async (
    ...
    try {
      ...
      const exchange =
        isOkx && Sdk.Okex.Addresses.Exchange[config.chainId]
          ? new Sdk.SeaportV15.Exchange(config.chainId, Sdk.Okex.Addresses.Exchange[config.chainId])
          : new Sdk.SeaportV15.Exchange(config.chainId);
      ...
      const conduit =
        isOkx && Sdk.Okex.Addresses.Exchange[config.chainId]
          ? toBuffer(
              new Sdk.SeaportV15.Exchange(
                config.chainId,
                Sdk.Okex.Addresses.Exchange[config.chainId]
              ).deriveConduit(order.params.conduitKey)
            )
          : toBuffer(
              new Sdk.SeaportV15.Exchange(config.chainId).deriveConduit(order.params.conduitKey)
            );
