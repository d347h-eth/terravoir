...
import { elasticsearchActivities as elasticsearch } from "@/common/elasticsearch";
...
import { getChainName, getNetworkSettings } from "@/config/network";
...
export const getTrendingMints = async (params: {
  ...
  period: Period;
  limit: number;
  mintStandards: string[];
}): Promise<ElasticMintResult[]> => {
  const { type, period, limit, mintStandards } = params;
  ...
  await Promise.all(
    Object.keys(results).map(async (period: string) => {
      ...
      if (mintStandards != null && mintStandards.length) {
        mintsQuery.bool.filter.push({
          terms: {
            ["collection.mintStandard"]: mintStandards,
          },
        });
      }
...
export const getActivityById = async (id: string): Promise<ActivityDocument | undefined> => {
  ...
  try {
    ...
    if (activities.length) {
      activityDocument = activities[0];
    }
  } catch {
    // logger.error(
    //   "elasticsearch-activities",
    //   JSON.stringify({
    //     topic: "get-by-id-error",
    //     data: {
    //       id,
    //     },
    //     error,
...
export const search = async (
  params: {
    ...
    excludeSpam?: boolean;
    excludeNsfw?: boolean;
    excludeChildActivities?: boolean;
  },
  debug = false,
  indexName?: string
  ...
  if (params.fungibles?.length) {
    (esQuery as any).bool.filter.push({
      terms: { type: [ActivityType.bridge, ActivityType.swap] },
    });

    const fungiblesFilter = {
      bool: {
        should: [
          {
            bool: {
              must: [
                {
                  terms: { "swap.fromCurrency.currency.contract": params.fungibles },
                },
              ],
            },
          },
          {
            bool: {
              must: [
                {
                  terms: { "swap.toCurrency.currency.contract": params.fungibles },
                },
              ],
            },
          },
    ...
    (esQuery as any).bool.filter.push(fungiblesFilter);
  }
  ...
  if (params.mintStandards?.length) {
    (esQuery as any).bool.filter.push({ term: { type: ActivityType.nftMint } });

    (esQuery as any).bool.filter.push({
      terms: {
        ["collection.mintStandard"]: params.mintStandards,
      },
    });
  }
  ...
  if (params.excludeChildActivities) {
    (esQuery as any).bool.filter.push({
      bool: {
      ...
      if (response?.errors) {
        ...
        if (errorItems?.length) {
          logger.error(
            ...
            JSON.stringify({
              topic: "updateActivitiesCollection",
              message: `Errors items in response`,
              data: {
                contract,
                tokenId,
                newCollection,
...
export const updateActivitiesParent = async (
  txHash: string,
  parentId: string
): Promise<boolean> => {
  let keepGoing = false;
    ...
    if (pendingUpdateDocuments.length) {
      const bulkParams = {
        body: pendingUpdateDocuments.flatMap((document) => [
          ...
          {
            script: {
              source: "ctx._source.parentId = params.parent_id;",
              params: {
                parent_id: parentId,
              },
            },
...
export const getCollectionVolumeSinceRecent = async (
  collection: string,
  ...
  (esAggs as any).volumeSinceRecent = {
    sum: {
      script: {
        source:
          "doc['pricing.price'].value != null ? Double.parseDouble(doc['pricing.price'].value) : 0",
      },
    },
  };
  ...
  const volumeSinceRecent = (esResult?.aggregations?.volumeSinceRecent as any).value;

  // eslint-disable-next-line @typescript-eslint/ban-ts-comment
  // @ts-ignore
  return Number(volumeSinceRecent);
};
