...
export const getListedTokensFromES = async (query: any, attributeFloorAskPriceAsObj = false) => {
  ...
  if (query.source) {
    ...
    query.sources = { operation: "include", ids: sourceIds };
  }
  ...
  if (query.nativeSource) {
    ...
    query.sources = { operation: "include", ids: [nativeSource.id] };
  }
  ...
  if (query.excludeSources) {
    ...
    const excludeSourceIds = query.excludeSources
      .map((source: string) => sources.getByDomain(source)?.id ?? 0)
      .filter((id: number) => id != 0);

    query.sources = { operation: "exclude", ids: excludeSourceIds };
  }
    ...
    if (query.includeDynamicPricing) {
      ...
      if (ask.order.rawData) {
        if (ask.order.isDynamic && ask.order.kind === "seaport") {
          ...
          ["sudoswap", "sudoswap-v2", "nftx", "nftx-v3", "zora-v4"].includes(ask.order.kind)
        ) {
          // Pool orders
          dynamicPricing = {
            kind: "pool",
