...
export const getTokensV7Options: RouteOptions = {
  ...
  validate: {
    query: Joi.object({
      ...
      sortDirection: Joi.string()
        ...
        .when("sortBy", {
          switch: [
            { is: Joi.string().valid("listedAt"), then: Joi.valid("asc", "desc").default("desc") },
            {
              is: Joi.string().valid("lastSaleAt", "lastSalePrice"),
              then: Joi.valid("desc").default("desc"),
            },
          ],
      ...
      tokenName: Joi.string()
        ...
        .when("collection", {
          is: Joi.exist(),
          then: Joi.when("sortBy", {
            is: Joi.string().valid("listedAt"),
            then: Joi.forbidden(),
            otherwise: Joi.allow(),
          }),
          otherwise: Joi.when("contract", {
      ...
      .when(".sortBy", {
        switch: [
          { is: Joi.string().valid("updatedAt"), then: Joi.any() },
          {
            is: Joi.string().valid("lastSaleAt", "lastSalePrice"),
            then: Joi.object().or("collection"),
          },
        ],
      ...
      .oxor("source", "nativeSource", "excludeSources"),
  },
  response: {
    schema: Joi.object({
  ...
  handler: async (request: Request) => {
    ...
    if (query.tokenName && query.sortBy === "listedAt") {
      logger.warn(
        `get-tokens-${version}-handler`,
        JSON.stringify({
          message: `tokenName and listedAt combo.`,
    ...
    let enableElasticsearchAsks = config.enableElasticsearchAsks;
    ...
    query.includeLastSale =
      query.includeLastSale || query.sortBy === "lastSaleAt" || query.sortBy === "lastSalePrice";
    ...
    if (query.nativeSource || query.excludeEOA) {
      ...
      if (query.nativeSource) {
        ...
        const nativeSource = sources.getByDomain(query.nativeSource, false);
    ...
    if (query.contract) {
      if (query.attributes && _.isArray(query.contract) && query.contract.length > 2) {
        throw Boom.badRequest(`No more than 2 contracts are allowed when filtering by attributes`);
      }
    ...
    if (query.excludeBurnt) {
      conditions.push(`(t.remaining_supply > 0 OR t.remaining_supply IS null)`);
    }
    ...
    if (query.minFloorAskPrice !== undefined) {
      ...
      conditions.push(
        `${query.nativeSource || query.excludeEOA ? "s." : "t."}${
          !(query.nativeSource || query.excludeEOA) && query.normalizeRoyalties ? "normalized_" : ""
    ...
    if (query.source) {
      ...
      (query as any).sources = query.source
        .map((source: string) => sources.getByDomain(source)?.id ?? 0)
        .filter((id: number) => id != 0);

      conditions.push(`t.floor_sell_source_id_int IN ($/sources:csv/)`);
    }
      ...
      if (
        ...
        switch (query.sortBy) {
          case "lastSalePrice": {
            ...
            query.sortDirection = query.sortDirection || "desc"; // Default sorting for lastSalePrice is DESC
            const sign = query.sortDirection == "desc" ? "<" : ">";
            if (contArr[0] !== "null") {
              conditions.push(
                `(t.last_sale_value, t.contract, t.token_id) ${sign} ($/contLastSaleValue/, $/contContract/, $/contTokenId/)
                  OR t.last_sale_value IS null`
              );
            } else {
              conditions.push(
                `(t.last_sale_value IS null AND (t.contract, t.token_id) ${sign} ($/contContract/, $/contTokenId/))`
              );
            }

            (query as any).contLastSaleValue = contArr[0];
            (query as any).contContract = toBuffer(contArr[1]);
            (query as any).contTokenId = contArr[2];
          ...
          case "lastSaleAt": {
            ...
            query.sortDirection = query.sortDirection || "desc"; // Default sorting for lastSaleAt is DESC
            const sign = query.sortDirection == "desc" ? "<" : ">";
            if (contArr[0] !== "null") {
              conditions.push(
                `(t.last_sale_timestamp, t.contract, t.token_id) ${sign} ($/contLastSaleAt/, $/contContract/, $/contTokenId/)
                  OR t.last_sale_timestamp IS null`
              );
            } else {
              conditions.push(
                `(t.last_sale_timestamp IS null AND (t.contract, t.token_id) ${sign} ($/contContract/, $/contTokenId/))`
              );
            }
            (query as any).contLastSaleAt = contArr[0];
            (query as any).contContract = toBuffer(contArr[1]);
            (query as any).contTokenId = contArr[2];
            break;
          }
    ...
    if (query.sortBy === "rarity" && query.sortDirection === "asc") {
      conditions.push(`t.rarity_rank IS NOT NULL`);
    }
    ...
    const getSort = function (sortBy: string, union: boolean) {
      ...
      switch (sortBy) {
        case "lastSalePrice": {
          return ` ORDER BY ${union ? "" : "t."}last_sale_value ${sortDirection} NULLS ${
        ...
        case "lastSaleAt": {
          return ` ORDER BY ${union ? "" : "t."}last_sale_timestamp ${sortDirection} NULLS ${
            sortDirection === "asc" ? "FIRST" : "LAST"
          }, t_contract ${sortDirection === "asc" ? "desc" : "asc"}, t_token_id ${
            sortDirection === "asc" ? "desc" : "asc"
          }`;
      ...
      if (
        ...
        switch (query.sortBy) {
          case "notListedTokens":
            ...
            break;
          case "lastSalePrice":
            continuation = rawResult[rawResult.length - 1].last_sale_price || "null";
            break;
          case "lastSaleAt":
            continuation = rawResult[rawResult.length - 1].last_sale_timestamp || "null";
            break;
          case "rarity":
    ...
    const result = rawResult.map(async (r) => {
      const feeBreakdown =
        r.top_buy_fee_breakdown?.map((f: any) => ({
          kind: f.kind,
      ...
      if (query.normalizeRoyalties && r.top_buy_missing_royalties) {
        for (let i = 0; i < r.top_buy_missing_royalties.length; i++) {
          const index: number = r.top_buy_fee_breakdown
            ? r.top_buy_fee_breakdown.findIndex(
        ...
        token: getJoiTokenObject(
          {
            ...
            mintedAt: new Date(r.minted_timestamp * 1000).toISOString(),
