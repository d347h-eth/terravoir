  ...
  handler: async (request: Request) => {
        ...
        return getJoiCollectionObject(
          {
            ...
            name: r.name,
            image: imageUrl ?? null,
            banner: Assets.getResizedImageUrl(
              r.banner,
              undefined,
              undefined,
              undefined,
    ...
    if (results.length >= query.limit) {
      ...
      if (lastCollection) {
        switch (query.sortBy) {
          case "1DayVolume": {
            continuation = buildContinuation(`${lastCollection.day1_volume}_${lastCollection.id}`);
            break;
          }

          case "7DayVolume": {
            continuation = buildContinuation(`${lastCollection.day7_volume}_${lastCollection.id}`);
            break;
          }

          case "30DayVolume": {
            continuation = buildContinuation(`${lastCollection.day30_volume}_${lastCollection.id}`);
            break;
          }
