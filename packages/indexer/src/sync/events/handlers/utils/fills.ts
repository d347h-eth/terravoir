  ...
  try {
    ...
    if (orderIds.length) {
      ...
      if (orders.length) {
        ...
        const orderIsReservoirByOrderId = new Map<string, boolean | null>();

        for (const order of orders) {
          orderSourceIdByOrderId.set(order.id, order.source_id_int);
          orderIsReservoirByOrderId.set(order.id, order.is_reservoir);
        }
        ...
        fillEvents.forEach((event) => {
          ...
          event.orderIsReservoir = orderIsReservoirByOrderId.get(event.orderId!) || undefined;
        });
      }
    }
  } catch (error) {
