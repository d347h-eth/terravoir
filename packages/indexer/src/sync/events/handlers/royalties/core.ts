...
export async function extractRoyalties(
  ...
  if (!fillEvents || (fillEvents && !fillEvents.length)) {
  ...
  if (!fillEvents.length) {
    throw new Error("fillEvents is empty");
  }
  ...
  for (const address in state) {
    ...
    if (balanceChange && !balanceChange.startsWith("-")) {
      ...
      if (feeRecipientPlatform) {
        ...
        if (linkedOrder) {
          const protocolFeeSumTmp = sameProtocolFills.reduce((total, item) => {
          ...
          if (protocolFeeSumTmp.gt(0)) {
            protocolFeeSum = protocolFeeSumTmp;
          }
        }

        // This is a marketplace fee payment
        // Reset the bps
        royalty.bps = protocolFeeSum.gt(0)
          ? bn(balanceChange).mul(PRECISION_BASE).div(protocolFeeSum).toNumber()
          : 0;
        ...
        if (shareSameRecipient && sameRecipientDetails.length) {
