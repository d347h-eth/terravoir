    ...
    queries.push(`
      WITH "x" AS (
        ...
        ON CONFLICT ("tx_hash", "log_index", "batch_index", "block_hash") DO UPDATE
          SET "order_id" = EXCLUDED.order_id, is_deleted = 0, updated_at = NOW() WHERE (fill_events_2.is_deleted = 1 OR fill_events_2.order_id IS DISTINCT FROM EXCLUDED.order_id)
