...
export const addEvents = async (events: Event[]) => {
  ...
  for (const event of events) {
    ...
    fillValues.push({
      ...
      order_is_reservoir: event.orderIsReservoir,
  ...
  if (fillValues.length) {
    ...
    queries.push(`
      WITH "x" AS (
        ...
        ON CONFLICT (tx_hash, log_index, batch_index, block_hash) DO UPDATE SET is_deleted = 0, updated_at = NOW() WHERE fill_events_2.is_deleted = 1
  ...
  await idb.manyOrNone(
    `
      ...
      WHERE block = $/block/
        ...
        AND is_deleted = 0
      RETURNING tx_hash, log_index, batch_index, order_id
    `,
    {
      block,
